...
# is: linestart, any whitespace (even none), image, any whitespace (even
# none), line ends.
...
class FigureCaptionProcessor(BlockProcessor):
    ...
    def test(self, parent, block):  # is the block relevant
        # Wenn es ein Bild gibt und das Bild alleine im paragraph ist, und das
        # Bild nicht schon einen figure parent hat, returne True
    ...
    def run(self, parent, blocks):  # how to process the block?
        ...
        captionText = self.FIGURES_RE.search(raw_block).group(
            1) or self.FIGURES_RE.search(raw_block).group(10)
